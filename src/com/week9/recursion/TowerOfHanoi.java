...
public class TowerOfHanoi {
    ...
    public static void main(String[] args){
        ArrayList<ArrayList<Integer>> result = towerOfHanoi(5);
    ...
    private static void TOH(int A, int s, int t, int d, ArrayList<ArrayList<Integer>> response) {
        if(A == 0) return;
        TOH(A-1,s,d,t,response);
        ArrayList<Integer> moves = new ArrayList<>();
        moves.add(A);
        moves.add(s);
        moves.add(d);
        response.add(moves);
        TOH(A-1,t,s,d,response);
