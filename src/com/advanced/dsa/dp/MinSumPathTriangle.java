...
public class MinSumPathTriangle {
    int[][] dp;
    public int minimumTotal(ArrayList<ArrayList<Integer>> a) {
        int n = a.size(),m = a.get(n-1).size();
        dp = new int[n][m];

        return ways(0,0,n,a);
    }

    public int ways(int i, int j,int n, ArrayList<ArrayList<Integer>> a){
        ...
        dp[i][j] = a.get(i).get(j) + Math.min(ways(i+1,j,n,a),ways(i+1,j+1,n,a));
